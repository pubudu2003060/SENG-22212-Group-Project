...
function UpdateStationStatus({ station, onUpdate }) {
  ...
  const handleOk = () => {
    ...
    axios
      .put(
        `http://localhost:8080/api/v1/updateStatus/${station.stationId}?status=${newStatus}`
      )
      .then((response) => {
    ...
    <>
      ...
      >
        <p>
          ...
          {station.status === "ACTIVE" ? "deactivate" : "activate"} this
          station?
        </p>
        <p>
          <strong>Registered ID:</strong> {station.registeredId}
        </p>
        <p>
          <strong>Owner's Name:</strong> {station.fuelStationOwner.name}
        </p>
      </Modal>
    </>
  );
