...
function InactiveFuelStations() {
    ...
    const fetchStations = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8080/api/v1/admin/filterByStatus?status=INACTIVE"
        );
        ...
        const uniqueLocations = Array.from(
          new Set(response.data.map((station) => station.location))
        );
        const uniqueStationTypes = Array.from(
          new Set(response.data.map((station) => station.stationType))
        );
        const uniqueFuelTypes = Array.from(
          new Set(response.data.map((station) => station.fuelType))
        );
  ...
  useEffect(() => {
    ...
    if (filters.location) {
      results = results.filter(
        (station) => station.location === filters.location
      );
    }
    ...
    if (filters.stationType) {
      results = results.filter(
        (station) => station.stationType === filters.stationType
      );
    }
    ...
    if (filters.fuelType) {
      results = results.filter(
        (station) => station.fuelType === filters.fuelType
      );
    }
                  ...
                  <td>
                    <UpdateStationStatus
                      ...
                      onUpdate={(stationId, newStatus) => {
                        ...
                        console.log(
                          `Station ${stationId} updated to ${newStatus}`
                        );
                      }}
                    />
                  </td>
