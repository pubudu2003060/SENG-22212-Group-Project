...
function Dashboard() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/api/v1/admin/getTotalActiveFuelStations")
      .then((response) => {
        setTotalActiveFuelStations(
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/api/v1/admin/getTotalVehicles")
      .then((response) => {
        setTotalVehicles(
          Array.isArray(response.data) && response.data.length === 0
            ? 0
  ...
  useEffect(() => {
    axios
      .get(
        "http://localhost:8080/api/v1/admin/findFuelStationCapacityBelow8000"
      )
      .then((response) => {
        setTotalLowFuelStations(
          Array.isArray(response.data) && response.data.length === 0
            ? 0
            : response.data
        );
            ...
            <Layout className="background_layout1">
              <Content className="background_content1">
                <Row gutter={[16, 16]} style={{ marginTop: "80px" }}>
                  <Col span={8}>
                    <Card
                      className="dashboard-card card-active-stations"
                      title="Total Active Fuel Stations"
                      bordered={false}
                    >
                  ...
                  <Col span={8}>
                    <Card
                      className="dashboard-card card-total-vehicles"
                  ...
                  <Col span={8}>
                    <Card
                      className="dashboard-card card-low-fuel-stations"
                      title="Low Fuel Stations"
                      bordered={false}
                    >
