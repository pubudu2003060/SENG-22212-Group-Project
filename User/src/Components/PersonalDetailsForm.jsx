...
function PersonalDetailsForm() {
  ...
  const validateForm = () => {
    const newErrors = {};
    const { idType, idNumber, phoneNumber, firstName, lastName, address } =
      formData;
    ...
    if (idType === "NIC") {
      ...
      if (!oldNicRegex.test(idNumber) && !newNicRegex.test(idNumber)) {
        newErrors.idNumber =
          "Invalid NIC. Old NIC: 9 digits + V/v. New NIC: 12 digits.";
      }
  ...
  const sendOTP = () => {
    if (
      ...
      /^07\d{8}$/.test(formData.phoneNumber)
    ) {
      axios
        .post(
          `http://localhost:8080/api/v1/login/send-otp/su/%2B94${formData.phoneNumber}`
        )
        .then((response) => {
    ...
    <div className="form-container">
      ...
      <form>
        ...
        <p className="profile-p">
          Please verify your phone number through the OTP code we have sent.
        </p>
